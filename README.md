...
2. **Point Rocket.Chat at it.** Set these in its environment, then start Rocket.Chat as usual:
   ...
   Now the body of every message are sealed before they reach the database.

3. **Grant a reader.** Sealed fields open only for a user the quorum granted the `crm-reader` role. Grant it to a member by their user id, from the minidauth checkout:

   ```sh
   DEMO_UID=<rocket.chat user id> ./bootstrap.sh
   ```

   Revoke it in minidauth and their reads go dark, with no change to Rocket.Chat.
<!-- minidauth-run:end -->
