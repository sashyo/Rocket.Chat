/**
 * Per-user reader identity for minidauth field opening in Rocket.Chat.
 *
 * The minidauth-seal sidecar holds NO reading identity of its own: it decrypts only on behalf of an
 * end user named in a token it verifies, and only if minidauth's quorum grant says that user holds the
 * reading role. The models layer has no request context, so we thread it ourselves: a Meteor method,
 * a REST handler, or the real-time streamer wraps its work in `withMinidauthReader(userId, fn)`, and
 * the seal layer opens sealed fields as that user. No reader in context means the field stays sealed:
 * ciphertext is the safe default, never an open decryption oracle.
 */
...
const b64url = (b: Buffer | string): string =>
	(Buffer.isBuffer(b) ? b : Buffer.from(b, 'utf8')).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
...
/**
 * Mint a short-lived assertion that this already-authenticated user is the reader. Rocket.Chat has
 * already verified the user (the id comes from the session, never from the client); this token just
 * carries that verified identity to the sidecar, which re-verifies the signature and decrypts as this
 * uid. minidauth's quorum grant decides whether the uid may actually read. Short-lived so a captured
 * token is useless within moments.
 */
...
export function withMinidauthReader<T>(uid: string | null | undefined, fn: () => T): T {
